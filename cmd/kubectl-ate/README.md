...
| `--endpoint-ca-file` | | With `--endpoint`: PEM CAs that verify the endpoint by its own name, or `system` for the system roots; no Kubernetes access is used, so `--token-file` is required | The cluster's ClusterTrustBundles |
