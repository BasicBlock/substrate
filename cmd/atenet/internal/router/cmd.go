...
func NewRouterCmd() *cobra.Command {
	...
	cmd.Flags().StringVar((*string)(&cfg.IngressAccess.Mode), "ingress-authorization", string(ingress.AccessDisabled), "How ingress authorizes clients: disabled (actors authenticate their own clients), audit (ask ate-api's CheckActorAccess about each client, log each it would deny, and forward it anyway) or enforce (answer 401 without a token that authenticates and 403 without can_connect on the actor, before resuming it)")
	cmd.Flags().StringSliceVar(&cfg.IngressAccess.TokenHeaders, "ingress-token-headers", []string{"ate-authorization"}, "Request headers a client's token may arrive in, in order of precedence (\"Bearer \" optional); put an identity-asserting proxy's header, such as x-goog-iap-jwt-assertion, first. They are removed before the request reaches the actor")
	cmd.Flags().StringSliceVar(&cfg.IngressAccess.StripHeaders, "ingress-strip-headers", nil, "More request headers to remove before a request reaches the actor, such as a proxy's credential header (proxy-authorization)")
	cmd.Flags().DurationVar(&cfg.IngressAccess.CacheTTL, "ingress-authorization-cache-ttl", ingress.DefaultAccessCacheTTL, "How long an allowed decision is reused for one token and actor, bounding how long a revoked binding still admits new requests (never past the token's expiry)")
