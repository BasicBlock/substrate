...
func (s *AteomService) gracefulShutdown(ctx context.Context) {
	...
	// atecontroller suspends the actors of a Terminating worker pod: each
	// CheckpointWorkload ends its actor's session, and the actor later resumes
	// on another worker from that snapshot instead of being stopped with this
	// pod. Give them a bounded head start. A checkpoint still running when the
	// wait ends is in flight, so the wait for in-flight RPCs below lets it
	// finish.
	if !s.awaitSessionEnd(ctx, time.Now().Add(min(*drainSuspendWait, workloadGracePeriod))) {
		slog.WarnContext(ctx, "Actors were not suspended while draining; stopping them", slog.Duration("waited", *drainSuspendWait))
	}
...
// awaitSessionEnd waits, until until, for every hosted actor's session to
// end, and reports whether they did. A checkpoint unhosts its actor
// (terminateWorkload), so the sessions end as the control plane suspends them.
func (s *AteomService) awaitSessionEnd(ctx context.Context, until time.Time) bool {
	for first := true; ; first = false {
		if len(s.hostedSessions()) == 0 {
			return true
		}
		if first {
			slog.InfoContext(ctx, "Waiting for the control plane to suspend the actors before shutting down", slog.Time("until", until))
		}
