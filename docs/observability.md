...
| `ate.actor.restore.duration` | atelet | histogram | how long each phase of a restore takes on the worker node, which is where cold-start latency actually goes once ateapi hands off (labels `ate.snapshot.phase`, `ate.snapshot.kind`, `ate.snapshot.scope`, `ate.template.atespace`, `ate.template.name`, `ate.sandbox.class`, plus `ate.restore.fallback` when a Full restore fell back to a Filesystem cold boot — see below) |
...
* `ate.snapshot.scope`: what content it covers. `full`, `filesystem` (gVisor only: rootfs plus durable data, no process memory), `data`, or `data_on_golden` (restore-only: the actor's data combined with the golden guest state).
...
**`ate.restore.fallback`** is present only on a restore that actually fell back — its absence means the normal path ran, so it never widens the label set of the common case. Today's only value is `full_to_filesystem`: a Full restore's memory restore (`runsc restore`) failed and the snapshot carried a Filesystem-scope image (a Full checkpoint captures one alongside the memory checkpoint on gVisor, best-effort), so atelet cold-booted from it instead of crashing the actor. When this fires, `ate.snapshot.scope` on the same datapoint reports the fallback's actual scope (`filesystem`), not the `full` scope the restore was requested with — the histogram describes what happened, not what was asked for.
