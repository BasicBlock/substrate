...
*   **Eviction:** When its worker pod is evicted, the controller suspends the actor, retrying a failed suspend with backoff until it succeeds, the actor no longer needs saving there (deleted, or not running), or the pod is gone — and it resumes on another worker from that snapshot on its next request. The worker waits up to `--drain-suspend-wait` (5 minutes by default) for the suspend to start. If none does, the actor gets `SIGTERM` and the rest of 30 minutes to be suspended; after that it is killed and moves to `ACTOR_STATE_CRASHED`. `kubectl ate revert actor` recovers a crashed actor by reverting it to `ACTOR_STATE_SUSPENDED` from its last external snapshot, discarding the execution since then; a later `resume` restores it from there — so only the work since that snapshot is lost, not the actor itself.
*   **Scale-down:** A WorkerPool scale-down (an HPA shrinking its `replicas`) deletes its Deployment's lowest-`pod-deletion-cost` pods first. The controller keeps that annotation on every worker pod — low for a free worker, high for one an Actor is bound to — so scale-down removes free capacity before it evicts a running Actor.
