...
*   **Eviction:** When its worker pod is evicted, the controller suspends the actor, and it resumes on another worker from that snapshot on its next request. The worker waits up to `--drain-suspend-wait` (2 minutes by default) for the suspend to start. If none does, the actor gets `SIGTERM` and the rest of 30 minutes to be suspended; after that it is killed and moves to `ACTOR_STATE_CRASHED`, and everything since its last snapshot is lost. A `CRASHED` actor can be recovered back to `ACTOR_STATE_SUSPENDED` at its last external snapshot using `RevertActor` (`kubectl ate revert`).
