...
A `WorkerPool` selects a **sandbox class** (`spec.sandboxClass`), and each class has a matching `ateom` herder image. The sandbox binaries themselves are not baked into the worker image — they, and the pause image holding the sandbox's namespaces, come at runtime from a cluster-scoped [`SandboxConfig`](api-guide.md#3-sandboxconfig-the-sandbox-itself) the `ActorTemplate` names in its sandbox config (naming one is currently required; per-class cluster defaults are planned) and are pinned into each snapshot's manifest so restores stay reproducible across runtime upgrades.
  ...
  * **gVisor** (`ateom-gvisor`, the default): Runs the workload under `runsc` for kernel-level sandboxing. Suspend and resume leverage gVisor's native checkpoint/restore of the sandboxed process tree. A `Full` checkpoint also captures a `Filesystem`-scope image (runsc's experimental `fscheckpoint`, alongside the memory checkpoint, best-effort) so a memory restore that fails — an incompatible CPU feature set across worker machine families, a restore-spec validation change, an old image — falls back to a cold boot from that image instead of failing the actor. A `Filesystem` snapshot can also be captured directly, or produced from a stored `Full` snapshot after the fact via `DropSnapshotMemory` (see [api-guide.md](api-guide.md#dropsnapshotmemory)), for a long-suspended actor whose memory snapshot's storage cost is no longer worth a hot-memory restore. `Filesystem` scope is gVisor-only for now: a micro-VM's `Full` snapshot already ships its own rootfs upper as a tar (see below), but the micro-VM class does not yet expose that as its own `Filesystem` scope or fall back to it on a failed memory restore.
